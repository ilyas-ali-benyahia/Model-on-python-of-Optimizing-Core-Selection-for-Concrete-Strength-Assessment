...
# Step 1: Randomly select 3 points from the data
        ...
        rmse_lista.append(rmse_iteration)
    
    if rmse_lista:
        mean_rmse = sum(rmse_lista) / len(rmse_lista)
        mean_rmse_dicta[Nc] = mean_rmse

mean_rmse_dict = {}

# Loop over each value of Nc
for Nc in Nc_values:
    rmse_list = []  # List to store RMSE for each iteration with current Nc
    
    for i in range(N):
        # Sort the DataFrame based on the 'RH' column
        df_sorted = df.sort_values(by='RH', ascending=True)
        
        # Divide the sorted DataFrame into Nc equally sized parts
        df_splits = np.array_split(df_sorted, Nc)
        
        # Randomly select one point from each part
        df_sample = pd.concat([split.sample(n=1, random_state=i) for split in df_splits])

        # Step 2: Fit a line to the selected points
        X_selected = df_sample[['RH']]
        y_selected = df_sample['F']
        model = LinearRegression().fit(X_selected, y_selected)
        ...
        # Step 3: Calculate Fe values for remaining RH values
        df_remaining = df.drop(df_sample.index)
        df_remaining['Fe'] = m * df_remaining['RH'] + b

        # Step 4: Calculate the error E = Fe - F
        df_remaining['E'] = df_remaining['Fe'] - df_remaining['F']

        # Step 5: Calculate RMSE for this iteration
        rmse_iteration = np.sqrt(np.mean(df_remaining['E'] ** 2))
        rmse_list.append(rmse_iteration)
    
    # Calculate mean RMSE for this Nc
    mean_rmse_dict[Nc] = np.mean(rmse_list)
...
plt.plot(list(mean_rmse_dicta.keys()), list(mean_rmse_dicta.values()), marker='o', color='g', label="Mean RMSE")
